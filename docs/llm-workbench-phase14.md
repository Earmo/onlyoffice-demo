...
| 能力 | 主路径 | 请求体 |
|---|---|---|
| 能力查询 | `POST /api/llm/capability/query` | `{"documentId":"demo"}` |
| 会话列表 | `POST /api/llm/sessions/list` | `{"documentId":"demo"}` |
| 创建会话 | `POST /api/llm/sessions/create` | `{"documentId":"demo","title":"可选标题"}` |
| 会话详情 | `POST /api/llm/sessions/detail` | `{"documentId":"demo","sessionId":"..."}` |
| 删除会话 | `POST /api/llm/sessions/delete` | `{"documentId":"demo","sessionId":"..."}` |
| 重命名会话 | `POST /api/llm/sessions/rename` | `{"documentId":"demo","sessionId":"...","title":"新标题"}` |
| 请求状态 | `POST /api/llm/requests/detail` | `{"documentId":"demo","requestId":"..."}` |
| 取消请求 | `POST /api/llm/requests/cancel` | `{"documentId":"demo","requestId":"..."}` |
