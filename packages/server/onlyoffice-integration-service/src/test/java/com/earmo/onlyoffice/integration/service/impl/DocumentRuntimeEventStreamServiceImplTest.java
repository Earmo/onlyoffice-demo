...
class DocumentRuntimeEventStreamServiceImplTest {
    ...
    @Test
    void shouldBindCapturedContextAroundDefaultLivenessTouch() {
        ScheduledExecutorService scheduler = Executors.newSingleThreadScheduledExecutor();
        CapturingRuntimeEventStreamService service = new CapturingRuntimeEventStreamService(
                scheduler,
                Clock.fixed(Instant.parse("2026-04-22T00:00:00Z"), ZoneOffset.UTC),
                0L,
                25000L,
        ...
        try {
            CurrentAccessContext.set(accessContext("user-a"));

            service.open("demo", status("demo", "initial"), () -> {
                assertEquals("user-a", CurrentAccessContext.actorUser());
            });

            CurrentAccessContext.clear();
            service.capturedLivenessTouch.run();

            assertNull(CurrentAccessContext.get());
        } finally {
            CurrentAccessContext.clear();
            scheduler.shutdownNow();
        }
    ...
    private static final class CapturingRuntimeEventStreamService extends DocumentRuntimeEventStreamServiceImpl {

        private Runnable capturedLivenessTouch;

        private CapturingRuntimeEventStreamService(
                ScheduledExecutorService keepaliveScheduler,
                Clock clock,
                long emitterTimeoutMillis,
                long keepaliveIntervalMillis,
                java.util.function.LongFunction<SseEmitter> emitterFactory
        ) {
            super(keepaliveScheduler, clock, emitterTimeoutMillis, keepaliveIntervalMillis, emitterFactory);
        }
        ...
        public SseEmitter open(
            ...
            capturedLivenessTouch = livenessTouch;
            return super.open(documentId, accessContext, initialStatus, () -> {
            });
        }
    }
}
