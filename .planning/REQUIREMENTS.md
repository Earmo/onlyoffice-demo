...
- [ ] **PH17-02**: 首次生成创建 assistant message + variant 0；重新生成在同一 assistant message 下新增 variant，不新增重复 user/assistant 消息
- [x] **PH17-03**: SSE、request status、session detail DTO 暴露 `variantId`、`variantIndex`、`activeVariantIndex` 和必要的 variants 数组
- [ ] **PH17-04**: prompt history 对每个 assistant message 只使用 active variant 文本
...
- [ ] **PH17-07**: 失败或取消的 regenerate variant 不破坏既有 completed 版本，并有自动化测试覆盖 active 行为
